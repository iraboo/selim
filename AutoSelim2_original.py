...
current_folder = 'D:/OneDrive/Documents/세림빌딩/월임대료 내역/'
...
#윈도우와 프레임 전환
browser.switch_to.window(browser.window_handles[1]) #다른창이 떠서 -1을 1로 변경, -1은 마지막창, 1은 두번째창 인듯
time.sleep(1)
...
p.append((browser.find_element(By.XPATH, '/html/body/div[3]/table/tbody/tr[1]/td[1]/table[1]/tbody/tr[12]/td')).text)   #청구요금
...
#엑셀파일 (임대료내역 파일형태)에 직접 전기/수도요금 저장
filename = current_folder + '2025-07.xlsx'
wb = openpyxl.load_workbook(filename)
ws_list = wb.sheetnames  #해당 Workbook의 시트 목록을 리스트로 저장

sheet = wb.get_sheet_by_name(ws_list[5])
sheet['E4'].value = e_price[0]
sheet['F4'].value = e_price[7]
sheet['G4'].value = -e_price[4]
sheet['C5'].value = e_price[5]
...
for i in range(5,12):
    wb = openpyxl.load_workbook(filename)
    sheet = wb.get_sheet_by_name(ws_list[i])
    pic = sheet._images[:]
    sheet._images.remove(pic[0])
    sheet._images.remove(pic[1])

    sheet.add_image(img1, 'A43')
    sheet.add_image(img2, 'E43')
    wb.save(filename)
...
''' 
#전기검침/수도검침 
e_check = [2930, 11473, 99885, 70345, 27748, 35515, 6994, 67389, 51586]    #전기검침 값
w_check = [452, 220, 406, 656, 4713]    #수도검침 값

sheet = wb.get_sheet_by_name(ws_list[3])
sheet.insert_rows(5,1)

sheet = wb.get_sheet_by_name(ws_list[4])
sheet.insert_rows(5,1)
'''
